...
case "$(uname -s)" in
    ...
    Linux*)     # Linux
        ...
        ;;
    *)          # Other
        echo "Unsupported OS for fzf auto-detection"
        ;;
esac
...
# Override fzf's host detection to only show meaningful hosts
__fzf_list_hosts() {
  # Only get hosts from .ssh/config (clean, meaningful names)
  cat ~/.ssh/config 2>/dev/null | command grep -i '^\s*host\s\+' | awk '{print $2}' | sort -u | command grep -v '[*?]'
  
  # Optionally add a few specific known_hosts entries (non-IP)
  command grep -oE '^[a-z][a-z0-9.-]+' ~/.ssh/known_hosts 2>/dev/null | tr ',' '\n' | awk '{print $1}' | sort -u | grep -v '^\([0-9]\|localhost\|broadcasthost\)'
}

# Enhanced hybrid ssh completion - fzf for hostnames, standard completion for flags
_fzf_complete_ssh() {
  # Use zsh completion variables to check previous word
  local prev_word=""
  
  # Check if there's a previous word and what it is
  if [[ $CURRENT -gt 2 ]]; then
    prev_word="${words[CURRENT-1]}"
  fi
  
  case "$prev_word" in
    # File completions - use fzf file completion
    -i|-F|-E|-S) 
      _fzf_path_completion "$prefix" "$1" ;;
    
    # Option completions - use standard ssh completion function directly
    -o|-c|-m|-O|-Q|-b|-D|-L|-R|-w|-J)
      # Call the original ssh completion function
      _ssh "$@" ;;
    
    # Hostname completion - use fzf with clean host list
    *) 
      local user
      [[ $prefix =~ @ ]] && user="${prefix%%@*}@"
...
# Register the hybrid completion
